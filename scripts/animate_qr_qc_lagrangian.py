...
def main():
    ...
    # Lagrangian dual: symlog handles 0 (loose budget) + large values (tight)
    lam_max = max(float(np.max(f["lambda"])) for f in frames)
    ax_lam.set_xlim(0, args.horizon)
    ax_lam.set_yscale("symlog", linthresh=1.0)
    ax_lam.set_ylim(0, max(lam_max * 1.1, 10.0))
    ax_lam.set_xlabel("step"); ax_lam.set_ylabel("λ$_i$")
    ax_lam.set_title("Lagrangian dual (active multiplier)"); ax_lam.grid(alpha=0.25, which="both")
