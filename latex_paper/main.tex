...
\textbf{At the zero-crossing regime} ($y^{\mathrm{tgt}}=0$, \cref{tab:fix_recovery_zero}), rejection and argmin drive $\sigma_{\mathrm{eff}}$ away from $0.7$ on turbines where the 1-step probe says the safe move would worsen DEL: recovered farm power is $+6.8\%$ (rejection) and $+5.6\%$ (argmin) above the linear blend, with DEL slightly reduced.
The recovery is modest here because the linear DEL excess is itself modest.
...
Full linear/rejection/argmin × $y^{\mathrm{tgt}}$ grids for all three actor-layout combinations appear in App.~\ref{app:full_sweep}; on MM (both actors) the linear curve has a clear DEL bulge centred at $y^{\mathrm{tgt}}=0$, and rejection/argmin never produce intermediate-yaw bulges.
...
$\beta$ must be chosen per layout per budget; without retraining for every constraint, the operator needs deployment-time composition---and then must avoid the hazard.
