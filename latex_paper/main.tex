...
We test four retraining baselines starting from the deployed actor on Safety Gymnasium PointGoal1 (SAC-Lagrangian at default, $\mu$-warmup, and aggressive-dual settings; SAUTE state-augmentation~\citep{sootla2022saute} which uses the same urgency variable as our schedule); at $500$k extra training steps, the best of them attains $R\!=\!2.3$ at $C\!=\!24$ ($d\!=\!25$), against the unconstrained $R\!=\!27$.
We instead propose a closed-form schedule $\sigma(u)$ that \emph{blends two existing controllers at inference}: the deployed performance policy $\pi_{\mathrm{perf}}$ and a certified safety controller $\pi_{\mathrm{safe}}$ (a factory wind-tracking yaw setpoint, an artificial-potential-field navigator).
...
On Safety Gymnasium ($\kappa\!\approx\!0.02$) it retains $45$--$71\%$ of unconstrained reward, dominating every retraining baseline we tested at every checkpoint $\in\{0, 50\text{k}, 200\text{k}, 500\text{k}\}$.
...
What the operator needs is a way to \emph{pace between} the aggressive RL policy and the certified safety controller so the cumulative constraint is satisfied, without retraining either.
...
The post-hoc blend dominates four distinct retraining baselines (SAC-Lagrangian at three hyperparameter settings, plus SAUTE) on SG at every compute checkpoint we measured (\S\ref{sec:sg_results}).
...
\begin{table}[h]
\centering
\caption{Safety Gymnasium with urgency-ratio APF blend (5 seeds $\times$ 20 ep). Unconstrained: 49.4 cost / 27.1 reward. Blend trades reward for cost as $\eta$ increases; at $d\!=\!40$ the budget is satisfied in 56\% of episodes with 71\% of unconstrained reward retained.}
\label{tab:safetygym}
\begin{tabular}{lccccc}
\toprule
...
Blend ($\eta\!=\!0$, no-op control) & 10 & $27.1\!\pm\!0.1$ & 100\% & $49.4\!\pm\!3.4$ & 12\% \\
...
\begin{table}[h]
\centering
\caption{Final-step ($500$k) baseline summary on Safety Gymnasium. The APF blend (zero retrain) dominates every retraining variant on reward.}
\label{tab:baselines}
\begin{tabular}{lcccc}
\toprule
Method & $d$ & Reward & Cost & Budget OK \\
\midrule
\textbf{APF blend (zero retraining)} & 10 & $\mathbf{12.3}$ & $18.6$ & --- \\
\textbf{APF blend (zero retraining)} & 25 & $\mathbf{15.8}$ & $28.6$ & --- \\
\midrule
...
Across both budgets, every retraining variant either violates the budget while preserving partial reward (warmup at intermediate checkpoints) or satisfies the budget by collapsing reward to $\approx\!0$ (aggressive Lagrangian, SAUTE).
The post-hoc blend dominates at every compute checkpoint we measured.
We do not claim retraining cannot \emph{eventually} reach a better reward-cost frontier with sufficient compute and tuning; we claim that, at the compute budgets a deployed-system operator can realistically afford, post-hoc blending is the better operating point.
