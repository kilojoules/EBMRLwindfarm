...
Nor does raising $\beta$ rescue the failing layout: a $\beta=2$ actor trained on stag4\_5d (90k steps, evaluation metrics converged) still exceeds the zero-yaw DEL baseline on the two upstream turbines ($+1.7\%$ each) while gaining $+8.4\%$ power---an improvement over $\beta=1$ (which exceeded baseline on all four turbines) but not Pareto dominance.
\emph{No tested $\beta$ achieves Pareto dominance on stag4\_5d.}
