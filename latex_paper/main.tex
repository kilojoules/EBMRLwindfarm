...
We identify a sharp, previously-unreported failure mode of this scheme on \emph{directional action manifolds}: when the controllers act on opposite sides of a non-convex cost ridge, the linear midpoint can have cost \emph{strictly higher than either endpoint}---the schedule is \emph{anti-Pareto in $\sigma$}.
...
On wind-farm yaw control with a physics-grounded blade-flap DEL surrogate~\citep{guillore2024sectoravg}, the predicted zero-crossing bump appears as a small local DEL maximum ($\approx 0.7\%$ above both immediate neighbours at $\sigma=0.7$, Table~\ref{tab:linear_hazard_mm}); a direct $\sigma$-sweep at fixed $\pi_{\mathrm{perf}}$ and a hazardous $\pi_{\mathrm{safe}}$ (the experiment matching the Proposition; Figure~\ref{fig:sigma_sweep}) shows whether the wind ridge is sharp enough to produce a substantive effect or whether the hazard is mainly a 2D-navigation phenomenon. % TODO(revision): figure pending sigma_sweep job 19012840
...
\caption{Cumulative DEL and farm power vs blend weight $\sigma$ at fixed $\pi_{\mathrm{perf}}$ (DEL-aware MM actor, settling at $\bar y \approx -15^\circ$) and fixed $\pi_{\mathrm{safe}}$. \textbf{Hazardous configuration} ($\pi_{\mathrm{safe}}=+15^\circ$, red): blend yaw crosses $0^\circ$ near $\sigma=0.5$ (dotted vertical), where the Proposition predicts a DEL bump. A local non-monotonicity in DEL appears at $\sigma=0.5$ (a $\approx+700$ kNm spike above the $\sigma=0.4$ neighbour) but the rest of the curve trends upward toward $\sigma=1$ because $\pi_{\mathrm{safe}}$ at $+15^\circ$ is itself a higher-DEL setpoint than $\pi_{\mathrm{perf}}$. \textbf{Co-directional control} ($\pi_{\mathrm{safe}}=-25^\circ$, blue): DEL is monotone in $\sigma$, as expected when the segment-convexity condition holds. % TODO(revision): when sigma_sweep_plus7p5.json (job 19013277) lands, add the +7.5 curve where both endpoints have lower DEL than the midpoint -- this would give the strict "above both endpoints" demonstration the abstract claims.
}
...
Our two-domain demonstration shows the failure is severe on 2D navigation ($-97\%$ goal reward at the worst-case rotation) and modest but visible in wind ($\approx 0.7\%$ DEL local bump above neighbours, $\approx 10\%$ local power loss at the zero-crossing); the magnitude depends on the sharpness of the cost ridge in each domain (we report a direct $\sigma$-sweep in wind to characterise this, \Cref{fig:sigma_sweep}). The cost-monotone fixes recover the Pareto frontier with one extra cost probe per step.
