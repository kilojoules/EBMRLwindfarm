...
We propose to \emph{blend two existing controllers at inference} using any monotone schedule $\sigma(u)\!\in\![0,1]$ of the \emph{urgency ratio} $u=\rho/\tau$ (remaining-budget / remaining-time fractions); on pace ($u\!\ge\!1$) the performance policy $\pi_{\mathrm{perf}}$ runs, in deficit ($u\!\to\!0$) the domain-specific safety controller $\pi_{\mathrm{safe}}$ takes over (a factory wind-tracking yaw setpoint, an artificial-potential-field navigator).
The closed-form $\sigma(u) = 1-\exp(-\eta(1/u-1))$ is one convenient single-parameter instantiation; in our wind-farm and Safety-Gymnasium ablations a hard-threshold switch performs comparably (Tables~\ref{tab:wf_ablation}, \ref{tab:ablations})---the work is in identifying $u$ as the controlling state variable, not in the specific form of $\sigma$.
\textbf{This is a budget-aware \emph{pacing} mechanism, not a hard enforcement guarantee.}
On wind-farm yaw control ($\kappa\!\approx\!0.72$) the schedule satisfies a $15$-step neg-yaw surrogate budget across 4 training seeds with $99.3$--$101.8\%$ of unconstrained power; in our 3-turbine setting zero yaw alone is power-competitive with the unconstrained policy, so WF validates budget tracking rather than preservation of a large wake-steering advantage.
We further validate against a physics-grounded blade-flap DEL surrogate (sector-averaged ANN on DLC12 simulations~\citep{guillore2024sectoravg}): a 4-point sweep of per-turbine budget scale ($B$, $B/2$, $B/3$, $B/5$ relative to a zero-yaw baseline calibration) shows a clean phase transition---$0/250$ episode violations across the feasible regime and $250/250$ at the infeasible $B/5$ point, with power retention essentially flat ($1.14$--$1.16\times$ unconstrained) where feasible.
...
\textbf{Honest gap.}
Our two domains co-validate the framework only piecewise: WF demonstrates budget tracking but not RL-policy preservation (zero yaw is power-competitive in our trained regime), while SG demonstrates policy preservation but not tight expected-cost feasibility.
A retrain with variable wind direction targeting both halves jointly is in progress; the current evidence supports each claim individually rather than jointly.
\end{abstract}
...
What the operator needs is a way to \emph{pace between} the aggressive RL policy and the domain-specific safety controller so cumulative cost can be tracked toward a budget target, without retraining either.
...
In our tested settings, the Lagrangian variants either preserve partial reward while violating the budget or drive cost down by collapsing goal-seeking; SAUTE's $\rho$-augmentation in our run does not by itself recover usefulness.
...
\textbf{Contributions.}
(1)~An \emph{urgency-ratio scheduler}: any monotone-decreasing $\sigma(u)$ defines a post-hoc pacing controller; we use a one-parameter exponential family but a hard-threshold switch performs comparably (\S\ref{sec:theory}; \S\ref{sec:wf_results}).
...
(3)~A \emph{gradient-correction variant} (\S\ref{sec:gradient}) where $\pi_{\mathrm{safe}}$ is implicitly a step of $-\nabla_a Q_c$. We propose the coupling strength $\kappa = \mathbb{E}\|\nabla_a Q_c\|/\mathbb{E}\|\nabla_s Q_c\|$ as a \emph{preliminary diagnostic} for when this variant is likely to fail (Table~\ref{tab:coupling}); we observe a $36\times$ gap between our two domains aligning with the qualitative outcome but do not claim broader validation.
...
When the segment $\pi_{\mathrm{perf}}\!\to\!\pi_{\mathrm{safe}}$ stays on one side of the DEL peak, cost is monotone in $\sigma$ (segment-convex condition of \cref{sec:theory} holds approximately).
...
\textbf{Central evidential gap.}
Our two domains co-validate the framework only piecewise.
Wind farm shows that the urgency schedule paces a per-turbine cumulative budget (binary neg-yaw and physics-grounded blade-flap DEL) with $0$ violations across hundreds of episodes, but in our trained regime pure $\pi_{\mathrm{safe}}$ (zero yaw) is power-competitive with the unconstrained policy, so there is no large RL advantage to preserve.
Safety Gymnasium is the converse: $\pi_{\mathrm{perf}}$ genuinely beats $\pi_{\mathrm{safe}}$ on reward ($R\!=\!27$ vs.\ $3.1$), but at tight budgets ($d\!=\!10, 25$) the blend's expected cost exceeds the budget and per-episode satisfaction is $23$--$56\%$, which is not operationally reliable.
We tried to close this gap with a $290{,}000$-step unconstrained retrain on \emph{stag4\_5d} and \emph{multi\_modal} (\S\ref{sec:both_halves}); at fixed wind direction the actor lands $1$--$4\%$ \emph{below} zero yaw on power.
A retrain with variable wind direction targeting both halves jointly is in progress; the present paper supports each claim individually rather than jointly.
...
We also do not claim the closed-form schedule is uniquely valuable: the WF and SG ablations (Tables~\ref{tab:wf_ablation}, \ref{tab:ablations}) show a hard threshold matches it within seed variance.
The contribution is the post-hoc-blending interface and the urgency variable as the controlling state, not the specific exponential form.
...
(2)~Our sim uses analytical wakes (Jensen) and 200-step episodes, not FAST.Farm aeroelastics over year-scale operation; a yearly damage budget is the operational target the urgency-ratio framing was designed for, but our experiments validate at episode scale only.
(3)~The APF navigator on SG is a simple reactive heuristic that uses hazard/goal geometry; a learned safety policy would likely improve $\pi_{\mathrm{safe}}$'s reward at fixed $\sigma\!=\!1$. The APF benefits from privileged geometry that the RL policy and retraining baselines do not directly use; the ablation table shares this privilege across all blend modes.
(4)~Action blending assumes meaningful convex combinations of $\pi_{\mathrm{perf}}$ and $\pi_{\mathrm{safe}}$ outputs. This holds for our continuous yaw and thrust/turn action spaces but may not for systems with discrete or non-convex action sets; in those cases a hard switch (Table~\ref{tab:ablations}) is more appropriate.
(5)~We test 4 seeds on WF and 5 on SG; retraining baselines use a single seed each at one hyperparameter setting per regime, covering three qualitatively different Lagrangian configurations (default, warmup, aggressive) plus SAUTE. Lagrangian methods are sensitive to reward scale and dual learning rate; we explored three settings, not a full sweep.
(6)~$\kappa$ depends on state and action scaling and on the quality of the learned $Q_c$. Comparing across two domains is suggestive, not definitive; broader validation across continuously varying coupling regimes (e.g.\ synthetic gridworlds) would strengthen the diagnostic claim.
(7)~Hard budget enforcement is not provided. A safe-future-cost upper bound $B_{\mathrm{safe}}(t)$ giving $C_t + B_{\mathrm{safe}}(t)\!\le\!d$ would be required and is left as future work.
(8)~The unconstrained EBT actor we trained does not Pareto-dominate zero yaw on power in this 3-turbine setting (\S\ref{sec:wf_results}); WF therefore validates budget tracking, not preservation of a large wake-steering benefit.
...
\textbf{Summary.}
A simple urgency variable $u=\rho/\tau$ provides a useful interface for post-hoc budget-aware arbitration between a pre-trained performance policy and a domain-specific safety controller.
The smooth closed-form schedule is one convenient single-parameter implementation; hard threshold switching is a competitive alternative (Tables~\ref{tab:wf_ablation}, \ref{tab:ablations}).
$\kappa$ provides a preliminary diagnostic for when local action-gradient correction is likely to fail, motivating explicit safety-controller blending.
At the compute budgets an operator can realistically afford, post-hoc blending preserves higher reward than the Lagrangian retraining baselines we tested---without their single-budget commitment.
The mechanism paces cumulative cost; hard enforcement requires an additional safe-future-cost bound (future work).
