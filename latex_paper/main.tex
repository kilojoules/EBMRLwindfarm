...
We identify a previously-unreported failure mode of this scheme on \emph{directional action manifolds}: when the controllers act on opposite sides of a non-convex cost ridge, the linear midpoint can have cost higher than either endpoint---the schedule is \emph{anti-Pareto in $\sigma$}.
The effect is sharp in 2D navigation (Safety Gymnasium PointGoal1: linear midpoint $C\!=\!483$, endpoints $C\!\le\!45$; \cref{tab:sg_hazard}) and present-but-marginal in wind-farm yaw control (a local $\approx 0.7\%$ DEL non-monotonicity at the predicted zero-crossing, within per-episode noise on 10 episodes; \cref{fig:sigma_sweep}).
...
On wind-farm yaw control with a physics-grounded blade-flap DEL surrogate~\citep{guillore2024sectoravg}, the predicted zero-crossing local DEL bump appears (\cref{tab:linear_hazard_mm}, \cref{fig:sigma_sweep}) but its magnitude is operationally marginal in our layout/wind setting: $\approx 0.7\%$ above immediate neighbours, within per-episode noise.
...
In wind, the zero-crossing bump is small in magnitude ($\approx 0.7\%$ local DEL bump above the immediate neighbours, $\approx 10\%$ local power loss versus the higher-power neighbour, \Cref{tab:linear_hazard_mm}); whether the wind ridge is sharp enough to matter operationally is the right question and we report a direct $\sigma$-sweep below (\Cref{fig:sigma_sweep}). % TODO(revision): figure depends on sigma_sweep job 19012840
...
We show this assumption can fail: the linear midpoint $a(\sigma)$ can have cost higher than both endpoints.
...
Any action that is a \emph{direction} or \emph{angle} (yaw setpoint, attack heading, steering angle, satellite pointing, gait phase) and any cost shaped like a ``ridge'' at the neutral direction expose this hazard.
...
\Cref{tab:linear_hazard_mm} shows that linear blend at $y^{\mathrm{tgt}}=0$ drives mean yaw to $\approx 0^\circ$ where blade-flap DEL is locally maximal: total DEL ($63375$) exceeds both immediate neighbours ($y^{\mathrm{tgt}}=-7.5^\circ$: $62913$ and $y^{\mathrm{tgt}}=+7.5^\circ$: $62708$) by $\approx 0.7\%$, and farm power is locally depressed (1.61~GW-step vs 1.79 and 1.53 at the neighbours, i.e.\ $-10\%$ vs the higher-power neighbour).
The effect is small relative to the swept extremes (the $y^{\mathrm{tgt}}=+25^\circ$ saturation regime, a separate mechanism, gives a much larger absolute DEL increase, $+4\%$).
\textbf{An important caveat:} the experiment in \cref{tab:linear_hazard_mm} varies $\pi_{\mathrm{safe}}$ \emph{direction} at fixed $\sigma=0.7$; the Proposition of \cref{sec:monotone_theory} is about non-monotonicity in $\sigma$ at fixed controllers.
A direct $\sigma$-sweep at fixed $\pi_{\mathrm{perf}}$ and a hazardous $\pi_{\mathrm{safe}}$ (i.e., $\pi_{\mathrm{safe}}$ on the opposite side of the cost ridge) is the experiment that matches the theorem; we report it below.
...
\textbf{Rejection and argmin recover Pareto at the zero-crossing hazard.}
The named hazard regime is $y^{\mathrm{tgt}}=0$: the blend forces yaw through the neutral DEL ridge.
At this regime (\cref{tab:fix_recovery_zero}), rejection and argmin both drive $\sigma_{\mathrm{eff}}$ away from $0.7$ on turbines where the 1-step probe says the safe move would worsen DEL, and the recovered farm power is $+6.8\%$ (rejection) and $+5.6\%$ (argmin) above the linear blend, with DEL slightly reduced.
The recovery is modest in this regime because the linear DEL excess is itself modest ($\approx 1\%$ above the lower endpoint).
% TODO(revision): author verify the y_tgt=0 row is the right canonical regime
% to lead with; +25 numbers (Table 4 below) are an order of magnitude bigger
% but a different mechanism (large-yaw wake saturation, not zero-crossing).

\begin{table}[h]
\centering
\caption{Alignment-hazard regime at the zero crossing ($\sigma=0.7$, $y^{\mathrm{tgt}}=0^\circ$, MM DEL-aware actor): linear blend through the DEL ridge vs cost-monotone fixes. Numbers from \cref{tab:sweep_mm_delaware} ($y^{\mathrm{tgt}}=0$ row).}
...
\midrule
Linear & 1.61 & 63375 \\
Rejection & 1.72 & 63000 \\
Argmin & 1.70 & 62692 \\
\midrule
\multicolumn{1}{l}{\emph{$\Delta$ rejection vs linear:}} & $+6.8\%$ pwr & $-0.6\%$ DEL \\
\multicolumn{1}{l}{\emph{$\Delta$ argmin vs linear:}} & $+5.6\%$ pwr & $-1.1\%$ DEL \\
\bottomrule
\end{tabular}
\end{table}

\textbf{A separate large-yaw effect.}
A second, distinct anti-Pareto regime appears at $y^{\mathrm{tgt}}=+25^\circ$, where the blend pushes yaw deep into the saturated-wake-misalignment region of the DEL surrogate.
This is a different mechanism from the zero-crossing alignment hazard---wake saturation, not segment-convexity violation at the neutral ridge---but it is also a regime where linear blending hurts and the gated fixes recover (\cref{tab:fix_recovery_sat}).
We report it here for completeness; it is not the hazard the paper is named for.

\begin{table}[h]
\centering
\caption{Saturated-wake regime ($\sigma=0.7$, $y^{\mathrm{tgt}}=+25^\circ$, MM DEL-aware actor): a separate large-yaw effect, not the zero-crossing alignment hazard. From \cref{tab:sweep_mm_delaware} ($y^{\mathrm{tgt}}=+25$ row).}
...
\midrule
\multicolumn{1}{l}{\emph{$\Delta$ rejection vs linear:}} & $+14.7\%$ pwr & $-3.8\%$ DEL \\
\multicolumn{1}{l}{\emph{$\Delta$ argmin vs linear:}} & $+15.3\%$ pwr & $-3.5\%$ DEL \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Direct $\sigma$-sweep: the experiment that matches the Proposition.}
The Proposition of \cref{sec:monotone_theory} concerns non-convexity of $C[a(\sigma)]$ in $\sigma$ at fixed $\pi_{\mathrm{perf}}, \pi_{\mathrm{safe}}$.
The direction sweep above is an indirect probe of this; the direct experiment fixes $\pi_{\mathrm{perf}}$ (the DEL-aware MM actor, settling at $\bar y \approx -15^\circ$) and $\pi_{\mathrm{safe}}$ (a $+15^\circ$ setpoint controller, so the blend yaw passes through $0^\circ$ at intermediate $\sigma$), and sweeps $\sigma\in\{0, 0.1, \ldots, 1.0\}$.
\Cref{fig:sigma_sweep} reports cumulative DEL and farm power against $\sigma$ for this hazardous configuration and, as a control, against a co-directional configuration ($\pi_{\mathrm{safe}}=-25^\circ$) where the segment-convexity condition is expected to hold.
% TODO(revision): figure depends on sigma_sweep job 19012840

\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/sigma_sweep.pdf}
\caption{Cumulative DEL and farm power vs blend weight $\sigma$ at fixed $\pi_{\mathrm{perf}}$ (DEL-aware MM actor, settling at $\bar y \approx -15^\circ$) and three $\pi_{\mathrm{safe}}$ choices, 10 episodes per $\sigma$.
\textbf{Red ($\pi_{\mathrm{safe}}=+15^\circ$, hazardous):} blend yaw crosses $0^\circ$ near $\sigma=0.5$; a local non-monotonicity ($\approx+700$ kNm spike) appears at $\sigma=0.5$, but the curve trends upward toward $\sigma=1$ because $\pi_{\mathrm{safe}}$ at $+15^\circ$ is itself a high-DEL setpoint, so the midpoint is not above the right endpoint.
\textbf{Green ($\pi_{\mathrm{safe}}=+7.5^\circ$, low-DEL endpoint):} both endpoints below the midpoint range, but midpoint DEL deviations sit within stochastic error.
\textbf{Blue ($\pi_{\mathrm{safe}}=-25^\circ$, co-directional control):} DEL is monotone in $\sigma$, as expected when the segment-convexity condition holds.
\textbf{Honest reading:} the wind DEL ridge at $0^\circ$ is real and the predicted local non-monotonicity is visible, but the magnitude in this layout/wind regime is within the per-episode noise floor; we did not find a $\pi_{\mathrm{safe}}$ direction in wind that gives a midpoint DEL strictly and significantly above both endpoints.
The strict-above-both effect predicted by the Proposition is clearly demonstrated in 2D navigation (Safety Gymnasium, \Cref{tab:sg_hazard}); in wind it is operationally marginal in our setting.}
...
Our two-domain demonstration shows the failure is severe on 2D navigation ($-97\%$ goal reward at the worst-case rotation, midpoint cost strictly above both endpoints) and present-but-marginal in wind (local $\approx 0.7\%$ DEL non-monotonicity at the zero crossing, within per-episode noise on 10 episodes; \Cref{fig:sigma_sweep}). The magnitude depends on the sharpness of the cost ridge in each domain. The cost-monotone fixes recover the Pareto frontier with one extra cost probe per step.
