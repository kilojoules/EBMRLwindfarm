...
Pure $\pi_{\mathrm{safe}}$ (zero yaw, no wake-steering) also retains 100.7\% of unconstrained power across 4 seeds---an unexpected finding that suggests the unconstrained actor's negative-yaw use is not contributing meaningful wake-steering benefit in this setting; we discuss this in \S\ref{sec:discussion}.
...
(8)~The unconstrained EBT actor's wake-steering benefit appears small in our experiments: pure $\pi_{\mathrm{safe}}$ (zero yaw) retains 100.7\% of unconstrained power (Table~\ref{tab:wf_ablation}). This indicates the actor we trained is not Pareto-optimal on the power axis, so the wind-farm experiments should be read as validating the \emph{blend mechanism's correctness} (correct fatigue budget tracking under various $\sigma$ choices) rather than as a demonstration that RL wake-steering yields large power gains here. A stronger $\pi_{\mathrm{perf}}$ (longer training, multi-layout, DEL-aware reward) would be required for that demonstration; the blend mechanism would compose with it identically.
