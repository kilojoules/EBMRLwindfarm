...
In our wind-farm experiment, sweeping $\pi_{\mathrm{safe}}$ direction at fixed $\sigma=0.7$ produces a clean U-shaped DEL bulge centred at the zero-crossing regime: at $y^{\mathrm{tgt}}=0$ the linear blend incurs $+1.1\%$ DEL above its better endpoint and $-15\%$ farm power (\Cref{fig:alignment_hazard}a, \Cref{tab:linear_hazard_mm}).
...
Our two-domain demonstration (yaw with blade-flap DEL, 2D navigation with rotated APF) shows the failure can be severe ($-15\%$ farm power at the zero-crossing regime; $-97\%$ goal reward on Safety Gymnasium) and that the cost-monotone fixes recover the Pareto frontier with one extra cost probe per step.
