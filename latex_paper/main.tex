...
\begin{figure}[h]
\centering
% TODO(revision): replace with results from scripts/sigma_sweep_test.py when job 19012840 completes
\fbox{\parbox{0.85\linewidth}{\centering\vspace{1.5em}\textbf{Figure pending.} Cost-vs-$\sigma$ sweep at fixed $\pi_{\mathrm{perf}}$ (DEL-aware MM actor) and two $\pi_{\mathrm{safe}}$ choices: $+15^\circ$ (hazardous: blend yaw crosses $0^\circ$) and $-25^\circ$ (co-directional: control). Generated by \texttt{scripts/sigma\_sweep\_test.py}; LUMI job 19012840.\vspace{1.5em}}}
\caption{Cumulative DEL and farm power vs blend weight $\sigma$ at fixed $\pi_{\mathrm{perf}}$ (DEL-aware MM actor) and fixed $\pi_{\mathrm{safe}}$. The hazardous configuration ($\pi_{\mathrm{safe}}=+15^\circ$) blends across the zero-yaw DEL ridge; the control ($\pi_{\mathrm{safe}}=-25^\circ$) does not. The Proposition (\cref{sec:monotone_theory}) predicts a hump in the DEL curve for the hazardous configuration and monotone behaviour for the control.}
